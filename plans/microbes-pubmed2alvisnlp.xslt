<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="MedlineCitation">
	...
	<a:section name="mesh" xpath-contents=".">
	  <a:feature name="UI" xpath-value="@UI"/>
	  <a:feature name="major" xpath-value="@MajorTopicYN"/>
	  <xsl:for-each select="../QualifierName">
	    <a:feature name="qualifier" xpath-value="."/>
	    <a:feature name="qualifier-UI" xpath-value="@UI"/>
	  </xsl:for-each>
	</a:section>
      </xsl:for-each>
      <xsl:for-each select="ChemicalList/Chemical/NameOfSubstance">
	<a:section name="chemical" xpath-contents=".">
	  <a:feature name="UI" xpath-value="@UI"/>
	</a:section>
      </xsl:for-each>
    </a:document>
